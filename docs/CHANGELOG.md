...
> Ultimo consolidamento `B2`: freeze esplicito application-side del failure-call contract sul percorso golden, così che il boundary `application` risulti difeso non solo sulla traduzione strutturata del failure del renderer con apex validato preservato, ma anche sull'invocazione del renderer con `zone_apex="testdomain.com"` e `PUBLIC_SAFE_FIXED_DNS_PROFILE` durante il failure path.
