...
> Ultimo consolidamento `B2`: freeze esplicito application-side dell'intero risultato strutturato di successo sul percorso non-golden, così che il boundary `application` risulti difeso non solo sul full-text ma sull'intero `BindZoneFileGenerationResult`.
