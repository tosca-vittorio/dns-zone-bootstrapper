...
> Ultimo consolidamento `B2`: freeze esplicito application-side del percorso non-golden contro il golden file dedicato, così che il boundary `application` risulti difeso anche da confronto testuale completo e non solo dalla parità cross-layer con il renderer puro.
