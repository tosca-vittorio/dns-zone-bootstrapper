...
- Ultimo consolidamento `B2`: congelato nel solo layer test del boundary `application` anche il failure-call contract sul percorso golden `generate_bind_zone_file("testdomain.com")`, verificando che, in presenza di `ValueError` interno del renderer, il risultato venga tradotto strutturalmente in `error_code="renderer_failure"` con `zone_apex` validato preservato, `zone_file_text=None` e invocazione del renderer congelata su `zone_apex="testdomain.com"` e `profile=PUBLIC_SAFE_FIXED_DNS_PROFILE`.
